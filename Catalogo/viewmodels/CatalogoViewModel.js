...
export class CatalogoViewModel {
    ...
    topGeral(qtd = 4) {
        return [...this.titulos].sort((a, b) => b.Score - a.Score)
                                .slice(0, qtd);
    }
    ...
    estatisticasPorTipo(tipo) {
        ...
        return {
            ...
            mediaPontuacao: lista.length
                ? (lista.reduce((acc, t) => acc + (t.Score || 0), 0) / lista.length).toFixed(1)
                : 0
        };
    }
    ...
    resumoGeral() {
        return [{
            ...
            totalEpisodios: this.titulos.reduce((acc, t) => acc + (t.Episodios || 0), 0),
            totalAssistidos: this.titulos.reduce((acc, t) => acc + (t.Assistidos || 0), 0),
            reassistidos: this.titulos.reduce((acc, t) => acc + (t.Reassistindo || 0), 0),
            ...
            mediaScore: this.titulos.length
                ? (this.titulos.reduce((acc, t) => acc + (t.core || 0), 0) / this.titulos.length).toFixed(1)
                : 0
        }];
    }

    dadosGraficoTipo() {
        const tipos = ["Serie","Filme","Documentário","Reality","Desenho","Show"];
        const valores = tipos.map(tipo => this.filtrarPorTipo(tipo).length);
        return { labels: tipos, data: valores };
    }

    dadosGraficoStatus() {
        const status = ["Assistindo","Reassistindo","Completado","Dropped","Planejado","Em-Espera"];
        const valores = status.map(s => this.filtrarPorStatus(s).length);
        return { labels: status, data: valores };
    }

    dadosGraficoPlataforma() {
        const plataformas = ["Netflix","Amazom Prime","Crunchroll","YouTube","MAX","Download","TV"];
        const valores = plataformas.map(p => this.titulos.filter(t => t.Onde === p).length);
